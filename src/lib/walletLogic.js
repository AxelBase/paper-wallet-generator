...
const wordlist = [
  ...
  'buzz','cabbage','cabin','cable','cactus','call','calm','camera','camp','can',
  ...
  'jar','jazz','jealous','jeans','jelly','jewel','job','join','joke','joy',
  ...
  'owner','oxygen','oyster','ozone','pact','page','pair','palace','palm','panda',
  'paragraph','park','parrot','party','pass','patch','path','patient','patrol','pattern',
  ...
  'physical','piano','picnic','picture','piece','pig','pill','pilot','pink','pioneer',
  'pipe','pistol','pitch','pizza','place','planet','plastic','plate','play','please',
  'pledge','pluck','plug','plunge','poem','poet','point','polar','pole','police',
  'pond','pony','pool','popular','portion','position','possible','post','potato','poverty',
  ...
  'quarter','question','quick','quit','quiz','quote','rabbit','racoon','race','rack',
  ...
  'refuse','region','regret','regular','reject','relax','release','relief','remove','render',
  ...
  'round','route','royal','rural','sad','saddle','sadness','safe','sail','salad',
  ...
  'sleeve','slight','slim','sling','slight','slice','slender','slice','slide','slight',
  ...
  'stand','standard','stare','state','steak','steel','stem','step','stereo','stock',
  ...
  'tray','treat','tree','trend','trip','trophy','trouble','truck','true','truth',
  ...
  'wage','wagon','wait','wall','walnut','want','warfare','warm','warrior','wash',
...
export function generateMnemonic(passphrase = '') {
  try {
    console.log('[DEBUG] generateMnemonic called');
    ...
    if (passphrase) {
      ...
      for (let i = 0; i < 16; i++) {
        entropy[i] ^= passBytes[i % passBytes.length];
      }
    }
    ...
    const words = [];
    for (let i = 0; i < 12; i++) {
      const idx = parseInt(bits.substring(i * 11, i * 11 + 11), 2);
      words.push(wordlist[idx]);
    }

    const result = words.join(' ');
    console.log('[DEBUG] Mnemonic:', result);
    return result;
  } catch (err) {
    console.error('[ERROR] generateMnemonic:', err);
    throw err;
  }
}
...
export async function generateRandomKeyPair() {
  try {
    console.log('[DEBUG] generateRandomKeyPair started');

    const privKey = secp.utils.randomSecretKey(); // Fixed: correct function name in v2+
    ...
    wifBytes[33] = 0x01;
    const h1 = sha256(wifBytes.subarray(0, 34));
    const h2 = sha256(h1);
    wifBytes.set(h2.subarray(0, 4), 34);
    const privateKey = bs58.encode(wifBytes);

    // Address
    const pubHash = sha256(pubKey);
    const rmd = ripemd160(pubHash);
    const addrBytes = new Uint8Array(25);
    addrBytes[0] = 0x00;
    addrBytes.set(rmd, 1);
    const ah1 = sha256(addrBytes.subarray(0, 21));
    const ah2 = sha256(ah1);
    addrBytes.set(ah2.subarray(0, 4), 21);
    const address = bs58.encode(addrBytes);

    console.log('[DEBUG] Address:', address);
    console.log('[DEBUG] Private WIF:', privateKey);

    return { privateKey, address };
  } catch (err) {
    console.error('[ERROR] generateRandomKeyPair:', err);
    throw err;
  }
}
...
export async function generateQR(data) {
  try {
    console.log('[DEBUG] generateQR called');
    return await QRCode.toDataURL(data, { errorCorrectionLevel: 'H', scale: 8 });
  } catch (err) {
    console.error('[ERROR] generateQR:', err);
    return '';
  }
}